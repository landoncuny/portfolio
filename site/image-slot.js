...
(() => {
  ...
  // ── PDF rasterization ───────────────────────────────────────────────────
  // A dropped PDF is rendered (first page) to a canvas via pdf.js, then
  // flows through the same WebP re-encode as raster drops. Loaded lazily
  // from CDN only when a PDF is actually dropped.
  let pdfjsP = null;
  function loadPdfjs() {
    if (pdfjsP) return pdfjsP;
    pdfjsP = import('https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.6.82/pdf.min.mjs')
      .then((m) => {
  ...
  async function pdfToDataUrl(file, targetW) {
    ...
    try {
      ...
      const scale = Math.min(4, cap / Math.max(base.width, base.height));
      const vp = page.getViewport({ scale: scale });
      const canvas = document.createElement('canvas');
      canvas.width = Math.max(1, Math.round(vp.width));
      canvas.height = Math.max(1, Math.round(vp.height));
      const ctx = canvas.getContext('2d');
      // White base — PDFs have no intrinsic background.
      ctx.fillStyle = '#fff';
  ...
  async function toDataUrl(file, targetW) {
    if (file.type === 'application/pdf') return pdfToDataUrl(file, targetW);
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      if (!file || ACCEPT.indexOf(file.type) < 0) {
        this._setError('Drop a PNG, JPEG, WebP, AVIF, or PDF file.');
        return;
      }
